// src/engine/WeaponSkillProjectile.ts
import { Camera }    from "./Camera";
import { BaseEnemy } from "./enemy/BaseEnemy";
import { distSq }    from "./Collision";

// ============================================================
// [🧱 BLOCK: Projectile Kinds]
// Only Q skills that need a visual/hit-check pass through here.
// ward/blink/wrath/iron/phantom are handled directly by
// WeaponSkillSystem without a projectile object.
// ============================================================
...
// ============================================================
// [🧱 BLOCK: WeaponSkillProjectile Class]
...
// leap      — instant impact AoE at landing point, no travel
// ============================================================
export class WeaponSkillProjectile {
  x:       number;
  y:       number;
  vx:      number;
  vy:      number;
  ...
  lifetime: number;          // ms — for instant AOEs set to 1
  ...
  // Chain counter — lightning only. Fireball repurposes this
  // field to carry its detonation AoE radius (see ConsumableSystem
  // legacy comment — same trick preserved here).
  chainsLeft: number = 0;
  ...
  update(deltaMs: number): void {
    ...
    // Instant AOEs (frost/void/leap) have lifetime=1 and no velocity
  ...
  draw(ctx: CanvasRenderingContext2D, camera: Camera): void {
    ...
    switch (this.kind) {
      ...
      // ── Leap — orange impact shockwave ────────────────────
      case 'leap': {
        ctx.beginPath();
        ctx.arc(sx, sy, this.radius * pulse, 0, Math.PI * 2);
        ctx.strokeStyle = `rgba(249,115,22,${pulse * 0.75})`;
        ctx.lineWidth   = 3;
        ctx.stroke();
        ctx.beginPath();
...
// ============================================================
// [🧱 BLOCK: Explosion VFX]
// Drawn by WeaponSkillSystem when fireball detonates.
// ============================================================
export class WeaponSkillExplosion {
  x:        number;
  y:        number;
  radius:   number;
  maxRadius:number;
